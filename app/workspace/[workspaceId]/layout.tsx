...
export default function WorkspaceLayout({ children }: WorkspaceLayoutProps) {
  ...
  const { pages, fetchPages, getPageTree, createPage } = usePageStore();
    ...
    <div className="flex h-screen bg-[var(--color-background)]">
          ...
          <div className="flex flex-col gap-0.5">
            {pageTree.map((node) => (
              <PageTreeItem
                ...
                node={node}
                level={0}
                currentPageId={params.pageId as string}
      ...
      {activeMenuNodeId && typeof document !== "undefined" && (
        <PageTreeMenuPortal
          ...
          onSelect={(type) => {
            handleCreateChildPage(activeMenuNodeId, type);
            handleCloseMenu();
          }}
...
function PageTreeMenuPortal({ position, onSelect, onClose }: PageTreeMenuPortalProps) {
  ...
  const menuContent = (
    <div
      ref={menuRef}
      className="fixed rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] py-1 shadow-lg min-w-[140px]"
      style={{
      ...
      {(["document", "canvas", "database"] as PageType[]).map((type) => (
        ...
        >
          ...
          <span className="capitalize">
            {type === "document" ? "Page" : type === "canvas" ? "Clearing" : type}
          </span>
        </button>
      ))}
    </div>
