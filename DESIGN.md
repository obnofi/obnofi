...
### Collaboration Cursor Palette

공유 편집 커서는 본문 하이라이트 팔레트와 분리된 4가지 전용 색을 사용한다.
...
- 각 클라이언트 세션은 위 4가지 중 **하나를 랜덤으로 고정 선택**한다.
- 문서 소유자와 공유 편집자 모두 동일한 규칙을 따른다.
- 커서 본체와 이름표 배경은 **항상 같은 색**을 사용한다.
- 이름표는 흰색 텍스트 + 흰색 보더 + 볼드 텍스트를 사용한다.

### Collaboration Cursor Variants

공유 편집 커서는 `apps/web/public/cursor/`의 PNG 에셋만 사용한다.

| Variant | 트리거 | 에셋 폴더 |
|---|---|---|
| `pointing` | 기본 상태 | `cursor/pointing/` |
| `highlighting` | `Cmd/Ctrl + X`를 누르고 있는 동안 | `cursor/highlighting/` |
| `fucku` | `h` 키를 누르고 있는 동안 | `cursor/fucku/` |

- 웹 앱 안에 있는 동안 전역 커서는 위 variant 중 하나로 유지된다.
- 원격 협업 커서는 로컬 커서와 **동일한 축소 비율과 hotspot 기준**으로 렌더링해야 한다.
- 문서 공유 편집에서 텍스트 caret awareness가 보이는 사용자는 페이지 포인터를 추가로 그리지 않는다.
